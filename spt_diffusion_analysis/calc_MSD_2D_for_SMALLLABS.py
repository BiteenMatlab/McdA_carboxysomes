...
pixel_size = 0.049
# define max time lag for MSD curve, must always be two less than min_frames
max_tau = 4
max_tau_for_fit = max_tau
# plot the single track MSD?
plot_single_track_msd = False
...
def calculate_msd2(x, y, max_tau):
...
# save the DF with the tracks that were analyzed
out_df_tracks = []
...
for file in track_files[:]:
    ...
    for track_id in df["TRACK_N"].unique():
        ...
        if n >= min_frames and gap == False:
            ...
            msd_curve, weights = calculate_msd2(track_x_coords, track_y_coords, max_tau)
            weights[weights == 0] = 1  # Avoid division by zero
            sigma = np.sqrt(1 / weights)
            
            # fit to MSD function
            popt, pcov = curve_fit(brownian, time_lags[:max_tau_for_fit], 
           ...
           # popt, pcov = curve_fit(anomalous_diffusion, time_lags, msd, sigma=sigma, absolute_sigma=True)
            ...
            msd_fitted = brownian(time_lags, *popt)
            # Residuals
            ...
            if r2 > 0.2:
                ...
                out_df_tracks.append(track_data)
                
                new_track_id +=1 
                new_track_id_col = [new_track_id for gg in range(n)]
                track_data["NEW_ID"] = new_track_id_col
            ...
            if plot_single_track_msd == True:# and n >= 4:
                ...
                ax = axes.ravel()
                ax[1].set_title(str(track_id) + ' ' + str(D) + ' ' + str(r2))
                ...
                ax[1].plot(time_lags, brownian(time_lags, *popt), '-',)
...
out_df_tracks_concat = pd.concat(out_df_tracks).reset_index()  
#out_df_tracks_concat.columns = ['FRAME_N','LOC_R','LOC_C','TRACK_N', 'ROI_NUM', 'MOL_ID', 'NEW_ID']
out_df_tracks_concat.to_csv(directory[:-5] + name + '_' + set_label + '_mintracklen-{}'.format(min_frames) + '_GOOD_TRACKS.csv', index = False)
...
dataDF.to_csv(directory[:-5] + name + '_' + set_label + '_mintracklen-{}_'.format(min_frames) + '_Dcoeff_SMT_tau4.csv', index = False)
...
# plot the data    
def plot_data(dcoeffs, track_lengths):
    ...
    x, bins, p = plt.hist(dcoeffs, bins=logbins, color='forestgreen', weights=track_lengths,
...
plot_data(d_out, track_length_out)
