...
def es1_model(r_2, D):
    ...
    return (1 - np.exp(-r_2 / (4*D*tau + 4*sigma**2)))

def es2_model(r_2, a, D1, D2):
   
    return (1 - (a*np.exp(-r_2 / (4*D1*tau + 4*sigma**2)) 
                + (1-a) * np.exp(-r_2 / (4*D2*tau + 4*sigma**2))))
...
def fit_diffusion_model(x_data, y_data, fit_model = 'es2'):
    ...
    if fit_model == 'es1':
      ...
      #  print("D: ", result.params['D'].value, result.params['D'].stderr)
        ...
        # Set up initial parameter values
        params = model.make_params(a = 0.5, D1 = 0.001, D2 = .01)
        ...
        params['D2'].min = 0.0001 # Set minimum limit for D
        ...
        # Set up initial parameter values
        params = model.make_params(a1 = 0.5, a2 = 0.5, D1 = 0.001, D2 = .01, D3 = .1)
        ...
        params['D1'].min = 0.0001  # Set minimum limit for D
        params['D1'].max = .01 # Set maximum limit for D
        params['D2'].min = 0.01  # Set minimum limit for D
        params['D2'].max = .1 # Set maximum limit for D
        params['D3'].min = 0.1  # Set minimum limit for D
...
def plot(data):
    ...
    fig, axes = plt.subplots(figsize=(2.7, 2.7), dpi=300) 
    ...
    #logbins = np.logspace(np.log10(min(data)), np.log10(max(data)),  bins=binBoundaries)
    #histogram, bins = np.histogram(data, bins=logbins)
    #normalized_histogram = histogram / np.sum(histogram)
    #plt.plot(bins[:-1], normalized_histogram, marker='o', ls='-',
    #         linewidth=1, markersize=3, color='gray')
    
    data_hist, bins = np.histogram(data, bins=binBoundaries) 
    logbins = np.logspace(np.log10(min(data)), np.log10(max(data)),  len(bins))
    x, bins, p = plt.hist(data, bins=logbins, color='forestgreen',
                            edgecolor='k', alpha=0.75)
   
      


    plt.xlabel('MSD (um)', fontsize=fontsize, ) 
...
for m in range(bootstraps):
    ...
    # sample dataset
    boostrapped_data =  all_r2_displacements#np.random.choice(all_r2_displacements, replace=True, size=int(len(all_r2_displacements)))
    ...
    cpds.append(cpd)    
    results.append(np.array([fit_result.params[key].value for key in fit_result.params]))
