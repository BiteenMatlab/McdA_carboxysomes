...
for ii, file in enumerate(filenames[:]):
    ...
    for rep in range(num_reps):
        ...
        data = ((df.iloc[:,rep+1].to_numpy()) / .1) * 60 
        y.append(data)
...
plt.xlabel('DNA (mg/mL)')
    
fig.tight_layout()
plt.savefig(directory[:-5] + 'DNA_dependence.svg', dpi=300) 
plt.show()