...
filenames = [file for file in filenames if 'Hn_cbcells' in file]
samples = []
dfs = []
...
fig, axes = plt.subplots(figsize=(2.5, 2.5), dpi=150) 
...
for ii, sample in enumerate(samples):
    ...
    data = sample_data['FOCI_PER_MICRON'].to_numpy().reshape(-1,1)
    print(len(data))
...
plt.xlabel('Foci per micron', fontsize=fontsize, ) 
plt.ylabel('Fraction of cells', fontsize=fontsize)
plt.yscale('linear')
plt.ylim(0, 0.5)
plt.xlim(0, 5)
fig.tight_layout()
plt.savefig(directory[:-5] + label + '_foci_per_micron.svg', dpi=300) 
plt.savefig(directory[:-5] + label + '_foci_per_micron.png', dpi=300) 
plt.show()